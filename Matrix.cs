...
namespace DeepLearn
{
    ...
    public class RealMatrix : Matrix<double>
    {
        ...
        public static RealMatrix operator >(RealMatrix c1, RealMatrix c2)
        {
            ...
            // Parallelize the outer loop to partition the source array by Height.
            for(int i=0;i<c1.Height;i++)
            {
                for (int j = 0; j < c1.Width; j++)
                {
                    result[i, j] = Convert.ToDouble(c1[i, j] > c2[i, j]);//bool->double
                }
            }
            return result;
        }
        ...
        public static RealMatrix Product(RealMatrix matrixA, RealMatrix matrixB)
        {
            ...
            if (aCols != bRows)//矩阵A的列=B的行，两个矩阵才能相乘
            {
            ...
            Parallel.For(0, aRows, i =>
            {
                for (int j = 0; j < bCols; ++j)    // each col of B                                  
                {
                    ...
                    {
                        result[i, j] += matrixA[i, k] * matrixB[k, j];//求和矩阵
                    }
                }
            });               
            return result;
        ...
        #region Scalar Operations 运算符操作
        ...
        #region Polymorphism generics compatibility 多态类兼容性
        ...
        public new RealMatrix Transpose//转置矩阵
        {
            get
            {
